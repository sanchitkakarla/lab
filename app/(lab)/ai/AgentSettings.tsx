...
import { Mic, MapPin, Clock, MessageSquare, Power, ChevronDown, Check } from 'lucide-react'
...
export function AgentSettings({ initial }: { initial: Partial<Settings> }) {
  const [settings, setSettings] = useState<Settings>({
    agent_enabled: initial.agent_enabled ?? true,
    ...
    <section className="mt-10">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={save}
          disabled={saving}
          className="flex items-center gap-1.5 text-xs font-medium px-4 py-2 rounded-full bg-gray-900 text-white hover:bg-gray-700 transition-colors disabled:opacity-50"
        >
          {saved ? <><Check className="w-3 h-3" /> Saved</> : saving ? 'Saving…' : 'Save changes'}
        </button>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="glass-card p-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">When off, the agent won't answer calls</p>
            </div>
          </div>
          <button
            onClick={() => set('agent_enabled', !settings.agent_enabled)}
            className={`relative w-11 h-6 rounded-full transition-colors ${settings.agent_enabled ? 'bg-gray-900' : 'bg-gray-200'}`}
          >
            <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${settings.agent_enabled ? 'translate-x-5' : 'translate-x-0'}`} />
          </button>
        </div>
        ...
        <div className="glass-card p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">The ElevenLabs voice used for calls</p>
            </div>
          </div>
              ...
              <div className="absolute top-full mt-1 left-0 right-0 glass-card p-1 z-20 shadow-lg">
                {VOICES.map(v => (
                  ...
                  >
                    ...
                    {v.id === settings.voice_id && <Check className="w-3.5 h-3.5" />}
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="glass-card p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">What the agent says at the start of every call</p>
            </div>
          </div>
          <textarea
        ...
        <div className="glass-card p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">Office location link</p>
              <p className="text-xs text-gray-400 mt-0.5">Google Maps or website URL shared with callers</p>
            </div>
          </div>
          <input
            type="url"
        ...
        <div className="glass-card p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">The agent tells callers these hours when asked</p>
            </div>
          </div>
          <input
            type="text"
